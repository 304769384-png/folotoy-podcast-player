...
esp_err_t bsp_button_init(bsp_btn_cb_t cb, void *user) {
    ...
    if (cb) {
        ...
        ESP_RETURN_ON_ERROR(adc_oneshot_config_channel(s_adc, BSP_BTN_ADC_CHANNEL,
                                                       &channel), TAG,
                            "direct button ADC channel configuration failed");
    }
    ...
    ESP_LOGI(TAG, "按键就绪:ADC1_CH%d %s", BSP_BTN_ADC_CHANNEL,
             cb ? "组件事件" : "直接采样");
    return ESP_OK;
}
