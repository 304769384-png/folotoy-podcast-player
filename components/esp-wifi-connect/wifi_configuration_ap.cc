...
{
    if (!fixed_ssid_.empty()) {
        // IEEE 802.11 SSIDs are limited to 32 bytes. The caller owns the UTF-8
        // content; truncate defensively so copying into wifi_config_t is safe.
        return fixed_ssid_.substr(0, 32);
    }
